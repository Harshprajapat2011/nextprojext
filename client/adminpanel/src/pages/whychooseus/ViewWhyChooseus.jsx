...
export default function ViewWhyChooseus() {
    ...
    let [whyChooseUslist, setWhyChooseUslist] = useState([])
    let [delAllid, setDelallid] = useState([])
    let [title, setTitle] = useState('')

    let whyChooseUsTable = () => {
        axios.get(`${apiBaseurl}/whyChooseUs/view`,
            ...
            .then((finalres) => {
                ...
                setWhyChooseUslist(finalres.whyChooseUsData)
    ...
    let multidel = () => {
        ...
        axios.post(`${apiBaseurl}/whyChooseUs/delete`, obj)
            .then((res) => {
                whyChooseUsTable()
            })
    }
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th className=' w-[20%] p-[10px_10px] text-left text-gray-400 font-normal'>DESCRIPTION</th>
                        ...
                        <tbody>
                        {whyChooseUslist.length >= 1 ?
                                whyChooseUslist.map((v, i) => {
                                    return (
                            <tr>
                                ...
                                <td className=' w-[40%] text-left p-[10px_10px] text-gray-400 font-normal'>{v.whyChooseUsTitle}</td>
                                <td className=' w-[20%] p-[10px_10px] text-left text-gray-400 font-normal'>
                                    <img src={staticPath+v.whyChooseUsImage}  alt="" className='w-[40px]' />
                                </td>
                            <td className=' w-[20%] p-[10px_10px] text-left text-gray-400 font-normal'>{v.whyChooseUsDescription}</td>
                            <td className=' w-[20%] p-[10px_10px] text-left text-gray-400 font-normal'>{v.whyChooseUsOrder}</td>
                                {v.whyChooseUsStatus ?
                                ...
                                <td className=' w-[10%] p-[10px_10px] text-left text-gray-400 font-normal'>
                                    <Link to={`/update-why-choose-us/${v._id}`}>
